...
add_action('send_headers', function ($wp){
	...
	if($module == 'json'){
		...
		if(strpos($action, 'mag.') === 0){
			...
			$response	= wpjam_api_response($wpjam_json);
...
function wpjam_send_json_header(){
...
function wpjam_api_response($wpjam_json){
	global $wp;
	...
	return apply_filters('wpjam_json', $response, $api_setting, $wpjam_json);
}
