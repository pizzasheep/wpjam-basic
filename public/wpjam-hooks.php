...
add_filter('pre_get_avatar_data', function($args, $id_or_email){
	...
	if(is_numeric($id_or_email)){
		...
		$user_id	= intval($id_or_email->post_author);
	...
	if($user_id){
		$user_avatar = get_user_meta($user_id, 'avatarurl', true);
		...
		if($user_avatar){
			...
			$args	= apply_filters('wpjam_default_avatar_data', $args, $user_id);
		}
	}
...
add_filter('get_avatar_comment_types', function($comment_types){
	return array_keys(WPJAM_Comment::get_types());
});
