...
function wpjam_get_current_user_id($required=true){
	$user_id	= apply_filters('wpjam_current_user_id', get_current_user_id());
	...
	if($user_id && !is_wp_error($user_id)){
		...
		if(wpjam_get_json()){
			if($required){
				wpjam_send_json(['errcode'=>'illegal_access_token', 'errmsg'=>'Access Token 为空或非法']);
			}else{
				return 0;
			}
		}else{
