...
class WEIXIN{
	...
	public function get_oauth_access_token($code){
		$user_oauth	= wp_cache_get($code, 'weixin_code');
		...
		if($user_oauth === false) {
			...
			wp_cache_set($code, $user_oauth, 'weixin_code', MINUTE_IN_SECONDS*5);	// 防止同个 code 多次请求
		}
	...
	public function get_tags($force=false){
		...
		$user_tags	= get_transient('weixin_user_tags');
		...
		if($user_tags === false || $force){
			...
			set_transient('weixin_user_tags', $user_tags, DAY_IN_SECONDS);
		}
	...
	public function get_tag_users($tagid, $next_openid=''){
		if(empty($next_openid)){
			$tag_users	= wp_cache_get($tagid, 'weixin_tag_users');
		}
		...
		if($next_openid || $tag_users === false){
			...
			if(empty($next_openid)){
				wp_cache_set($tagid, $tag_users, 'weixin_tag_users', MINUTE_IN_SECONDS);
			}
		}
	...
	public function get_blacklist($next_openid=''){
		if(empty($next_openid)){
			$blacklist	= get_transient('weixin_blacklist');
		}
		...
		if($next_openid || $blacklist === false){
			...
			if($next_openid == ''){
				set_transient('weixin_blacklist', $blacklist, HOUR_IN_SECONDS);
			}
		}
	...
	public function batch_blacklist($openid_list){
		delete_transient('weixin_blacklist');
	...
	public function get_material($media_id, $type='image', $force=false){
		...
		if($type=='image' || $type=='thumb'){
			...
			if($material === false || $force){
				...
				wp_cache_set($media_id, $material, 'weixin_material', DAY_IN_SECONDS);
			}
			return $material;
		}elseif($type == 'video'){
			$response	= $this->http_request($url, array(
	...
	public function get_material_count(){
		...
		if($material_count === false){
			...
			set_transient('weixin_material_count', $material_count, 60);
		}
	...
	public function get_js_api_ticket(){
		$response = get_transient('weixin_js_api_ticket');
		if($response == false){
			...
			set_transient('weixin_js_api_ticket', $response, $response['expires_in']);
		}
	...
	public function get_wx_card_ticket(){
		$response = get_transient('weixin_wx_card_ticket');
		if($response == false){
			...
			set_transient('weixin_wx_card_ticket', $response, $response['expires_in']);
		}
	...
	public function get_customservice_kf_list(){
		$kf_list	= get_transient('weixin_kf_list');
		if($kf_list === false){
			$response	= $this->http_request('https://api.weixin.qq.com/cgi-bin/customservice/getkflist');
			if(is_wp_error($response)){
				set_transient('weixin_kf_list', [], 60);
				return $response;
			}else{
				$kf_list = $response['kf_list'];
				set_transient('weixin_kf_list', $kf_list, 3600);
			}
		}
	...
	public function get_access_token($force=false){
		$response = get_transient('weixin_access_token');
	...
	public function clear_quota(){
		...
		if($last_clear_quota === false){
			set_transient( 'weixin_clear_quota', 1, HOUR_IN_SECONDS);
