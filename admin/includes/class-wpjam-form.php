...
class WPJAM_Form{
	...
	public static function parse_field($field){
		...
		foreach ($field as $attr_key => $attr_value) {
			...
			if(!in_array($attr_key, ['type','name','title','key','description','class','value','default','options','fields','size','show_admin_column','sortable_column','taxonomies','taxonomy','datalist','settings','data_type','item_type','total','field_callback','field_validate','column_callback','extra'])){
