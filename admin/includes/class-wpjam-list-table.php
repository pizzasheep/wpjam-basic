...
class WPJAM_List_Table extends WP_List_Table {
	...
	public function get_filter_link($filters, $title, $class=''){
		...
		return '<a title="'.esc_attr($title_attr).'" href="javascript:;" class="list-table-filter '.$class.'" data-filter=\''.wpjam_json_encode($data_filters).'\'>'.$title.'</a>';
	}
	...
	public function extra_tablenav($which='top') {
		...
		if($which == 'top'){
			...
			if($actions){
				...
				foreach ($actions as $action_key => $action) {
					if(!empty($action['overall'])){
						$overall_actions	.= $this->get_row_action($action_key, ['class'=>'button-primary button']);
					}
				}
